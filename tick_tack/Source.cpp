...
void Move(char field[], const int n, char player)
{
	cout << "Игрок который играет " << player << endl;
	char key;
	do
	{
		...
		key = _getch();
		if (key == 27)return;
		if (key < '1' || key>'9') cout << tab << " Выход за пределы игрового поля " << endl;
		else if (field[key - 49] != 0) cout << tab << " Ошибка: Клетка уже занята " << endl;
